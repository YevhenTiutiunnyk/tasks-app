      ...
      {tasks.length === 0 ? (
        ...
        <ul>
          {tasks.map((task) => {
            ...
            // вхождение серии, а не собственная строка в tasks. На этом экране
            // ветка сейчас недостижима: все три секции идут из
            // getChecklistTasks(horizon 'week' | 'month'), а
            // expandRecurrences разворачивает повторы только в горизонт 'day',
            // так что среди задач чеклиста вхождений серии не бывает. Оставлена
            // как предохранитель на случай, если чеклист когда-нибудь снова
            // станет показывать дневной горизонт, — тогда PATCH /api/task
            // по-прежнему откажет в отметке такого id, и здесь опять понадобится
            // текст без интерактива вместо галочки.
