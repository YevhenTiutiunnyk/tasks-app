...
run('изоляция по владельцу', () => {
  ...
  // Пока тесты ходят в боевую базу, единственная защита — предохранитель
  // ниже: он считает чужие записи журнала ДО вызова и роняет прогон, если
  // они есть. На боевой базе это значит, что тест не идёт вовсе, и так
  // и задумано — до отдельной тестовой базы у него нет безопасного запуска.
  describe('обрезка журнала', () => {
    it('оставляет по пятьдесят записей каждому, а не пятьдесят на всех', async () => {
      ...
      // ПРЕДОХРАНИТЕЛЬ, и падать он обязан ДО applyOperations.
      //
      // Ниже по тексту вызывается боевая обрезка журнала. Пока фильтр владельца
      // в ней цел, чужие записи в безопасности — но проверяем мы здесь ровно
      // то, что он может отвалиться, и тогда обрезка снесёт чужое до того, как
      // хоть одно утверждение об этом успеет сработать.
      //
      // Порога «не больше стольки-то» или «все свежее такой-то даты» тут
      // недостаточно, и это не теория: достаточно потерять внешний фильтр,
      // оставив внутренний, и запрос превращается в «удалить всё, кроме моих
      // полусотни свежих» — без оглядки на возраст и количество. Ни один
      // порог такого не переживёт.
      //
      // Поэтому условие ровно одно: чужих записей в журнале быть не должно
      // вовсе, то есть база — отдельная тестовая, а не боевая. На боевой этот
      // expect валит тест намеренно: лучше красный прогон, чем молча стёртая
      // история отмены живого человека. Именно так 19.08 пропали 28 записей —
      // спас снятый заранее дамп, которого в следующий раз может не быть.
      expect(
        before.c,
        'Журнал содержит чужие записи — база не тестовая. Тест не запускается: '
        + 'обрезка в applyOperations может стереть их до первой проверки.',
      ).toBe(0);
