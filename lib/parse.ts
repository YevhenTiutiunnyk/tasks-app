...
function buildSystemPrompt(input: ParseInput): string {
  ...
  return [
    ...
    '8. Горизонт при создании: если дело названо без дня, но с периодом («на этой неделе», «в этом месяце») — ставь horizon="week" или "month", date — любую дату внутри периода, allDay=true, startMinute=null, durationMinutes=null. Такие задачи в needsTime НЕ добавляй: у них нет времени по замыслу. Во всех остальных случаях при создании — horizon="day".',
    'Горизонт при правке переходит вместе с датой, а не отдельно от неё — операция ставит оба поля разом или не трогает ни одно. Если фраза называет существующей задаче конкретный день или час («сделаю кран в четверг», «в четверг в 10») — это переход в дневной горизонт, даже если раньше у задачи была неделя или месяц: ставь horizon="day" и date равной этому дню. Если фраза называет задаче другой период («перенеси кран на следующий месяц») — ставь новый horizon и date — любую дату внутри НОВОГО периода, а не прежнюю дату задачи. Если фраза не называет ни дня, ни периода — horizon оставляй null, это значит «не трогать», как и у остальных полей правки.',
