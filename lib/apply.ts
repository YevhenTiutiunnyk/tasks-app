  ...
  return sql.begin(async (tx) => {
    ...
    for (const operation of operations) {
      ...
      if (operation.type === 'delete') {
        // Фильтр здесь дублирует выборку выше и остаётся намеренно — но не
        // потому, что владелец строки может смениться между запросами: такого
        // пути в коде нет, user_id у задачи не меняется нигде. Он второй рубеж
        // на случай регресса первого. Мутационная проверка это показала: снять
        // фильтр в выборке выше — и пачка перестаёт падать на чужой задаче, но
        // задача остаётся цела, потому что удаление до неё не дотягивается.
        // Снять оба — и чужое занятие действительно удаляется.
