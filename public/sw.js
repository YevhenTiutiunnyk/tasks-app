...
self.addEventListener('notificationclick', (event) => {
  ...
  event.waitUntil(
    (async () => {
      ...
      // Поэтому сначала navigate, потом focus — но только когда экран и
      // правда другой. У напоминания url всегда '/', и если человек уже
      // на нём стоит, navigate() на тот же адрес — это полноценная загрузка
      // документа, а не переход внутри приложения: App Router перемонтируется,
      // неделя перезапросится заново, а всё, что набрано в CommandBar,
      // пропадёт. До этой ветки напоминание делало только focus() — тап по
      // нему не должен перезагружать экран, на котором и так то, что нужно.
      for (const client of windows) {
        if ('focus' in client) {
          ...
          try {
            ...
            // Непрозрачный (opaque) клиент — сравнить не с чем, ведём себя
            // как раньше и пробуем navigate.
          }
          if ('navigate' in client && here !== url) {
            try {
