...
export function SeriesChoiceDialog({ action, note, onChoose, onCancel }: Props) {
  return (
    <div className="fixed inset-0 z-30 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-xs rounded-xl bg-white p-4 dark:bg-neutral-900">
        <h2 className="mb-1 text-base font-semibold">Это повторяющаяся задача</h2>
        <p className="mb-2 text-xs opacity-65">Что именно {action}?</p>
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            className="w-full rounded-md border border-neutral-500/30 px-3 py-2 text-sm"
          >
            Всю серию
          </button>
          <button onClick={onCancel} className="w-full px-3 py-1.5 text-xs opacity-60">
            Отмена
          </button>
        </div>
      </div>
