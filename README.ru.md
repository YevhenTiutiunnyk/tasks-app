...
стартовать, если `DATABASE_URL` задана, а `TEST_DATABASE_URL` не ведёт
в по-настоящему другую базу, так что такую ошибку и совершить-то не получится.
...
`DATABASE_URL` задана, а `TEST_DATABASE_URL` — нет или ведёт в ту же самую базу:
потерянная или неверная тестовая переменная падает громко, а не тихо утекает
в боевую базу. «Та же самая» определяется разбором обеих строк и сравнением
хоста и имени базы, а не сравнением строк: `postgres://` против `postgresql://`,
лишний `?sslmode=require` или второй порт Supabase — это одна и та же база,
...
при этом локальный, в контейнере, — полный прогон занимает около секунды.
